	...
	deployer.deploy(ARCToken,
		...
		deployer.then(function() {
			...
			// function TokenVesting(address _tokenRecepient, uint _freezePeriod, uint _initialAmount, uint _period,  uint _amount,address _tokenContract){
 			return TokenVesting.new(founder_address, 10, 10, 10, 10, CATToken.address);
		}).then(function(founder_vesting) {
			deployer.then(function() {
				// developer vesting contract
				return TokenVesting.new(developer_address, 10, 10, 10, 10, CATToken.address);
			}).then(function(developer_vesting) {
				deployer.then(function() {
					// rewards vesting contract
					return TokenVesting.new(rewards_address, 10, 10, 10, 10, CATToken.address);
					...
					// return CATToken.deployed().setRewardAddresses(founder_vesting.address, developer_vesting.address, rewards_vesting.address).then(function() {
					// 	console.log('set reward addresses');
						

					// 	return CATToken.deployed().founder().then(function(q) {
					// 		console.log('CAT founder reward address=', q);
					// 	});
					// 	CATToken.deployed().developer().then(function(q) {
					// 		console.log('CAT developer reward address=', q);
					// 	});
					// 	CATToken.deployed().rewards().then(function(q) {
					// 		console.log('CAT rewards reward address=', q);
					// 	});
					// });
